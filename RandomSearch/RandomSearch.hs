...
type CostPair = (Float, [Float])

randomSearch :: (Float -> Float) -> (CostPair -> CostPair -> CostPair) -> Int -> Float -> Float -> Int -> Int -> CostPair
-- In the search the list of random numbers is split into groups of length equal to
-- the number of dimensions and the cost function is applied to each group.
	...
	let randNums = randomList seed min max;
	    ...
	    costs nDims lst 0 = []
            costs nDims lst iters = let (candidate, rest) = splitAt nDims lst
			            in (cost costFn candidate, candidate) : costs nDims rest (iters - 1) 
        in foldr1 optimizeFn (costs nDims randNums iters)

minCost :: CostPair -> CostPair -> CostPair
